...
class AdminPage:
	...
	def login(self):
		...
		# 数据库操作 查询管理员表
		db = pymysql.connect("localhost", "root", "zxcvbnm1999", "pythondb")  # 打开数据库连接
...
class AdminManage:
	def __init__(self, parent_window):
		...
		self.age = []
		# 打开数据库连接
		db = pymysql.connect("localhost", "root", "zxcvbnm1999", "pythondb") 
		cursor = db.cursor()  # 使用cursor()方法获取操作游标
		...
		try:
			...
			messagebox.showinfo('警告！', '数据库连接失败1！')
		db.close()# 关闭数据库连接
...
if __name__ == '__main__':
	try:
		...
		messagebox.showinfo('错误！', '连接数据库失败5！')
